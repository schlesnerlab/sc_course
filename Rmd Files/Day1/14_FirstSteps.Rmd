...
We will be analyzing a dataset of Peripheral Blood Mononuclear Cells (sample) freely available from 10X Genomics.
...
The raw data can be found [here](https://cf.10xgenomics.com/samples/cell/sample3k/sample3k_filtered_gene_bc_matrices.tar.gz), and you have already downloaded it with the repository Zip file.
...
# Load samples.
mt_pattern <- "^MT-"
sample_path <- "/omics/groups/OE0436/internal/heyer/scourse/sc_course/data/filtered/BT322.filtered_gene_matrices" # Path to where the count matrix from cellranger (either filtered or raw, your choice) is located.
...
sample@meta.data$orig.ident %>% unique()
sample <- sample[,sample$orig.ident == "OE0145-IDH_NCH6341" ]
# Compute percentage of mitochondrial RNA in the cells.
sample[["percent.mt"]] <- Seurat::PercentageFeatureSet(object = sample,assay = "RNA",
                                                       pattern = mt_pattern)
...
pheatmap::pheatmap(log10(sample[1:500, 1:200]@assays$RNA$counts + 1),
  show_rownames = F,
  show_colnames = F,
  cluster_rows = F,
  cluster_cols = F,
...
> Hint: subset the assay data, `sample@assays$RNA`.
