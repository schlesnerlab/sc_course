...
```{r}
do_ViolinPlot(sample, features = "Oligodendrocytes", assay = "panglao_aucell")
```
...
To solve this I recommend you check the GSE object we use on Day three. 
Compare those annotations with your annotations. 
...
net$source %>% unique() # Exercise Answer
...
```{r}
p3 <- (do_NebulosaPlot(sample, features = c("p53"),) & 
  scale_colour_gradient2(low = 'blue', mid = 'white', high = 'red')) +
  ggtitle('MAPK activity')
p3
do_FeaturePlot(sample, features = c("p53", "NFkB"),individual.titles = c("p53", "NFkB"))
...
```{r}
# EC Marker example
do_FeaturePlot(sample, features = c("A2M", "FLT1", "CAVIN2", "VWF"), assay = "RNA")

# OPC-like
DefaultAssay(sample) <- "RNA"
do_NebulosaPlot(sample, features = c("ACAP3", "TNK2", "SLC1A1", "RGR"),)
