...
We'll be working with the data from "First steps", in case you didnt save the work form last time, you need to reread the data and process it again.
...
sample <- FindNeighbors(sample, dims = seq_len(15), verbose = FALSE)
...
When working with your own data, especially with larger datasets it is important to 
...
```{r}
sample <- RunUMAP(sample, dims = 1:10, n.neighbors = 60, verbose = FALSE,
                  reduction.name = "umap_nn60" )
do_DimPlot(sample, reduction = "umap_nn60") # Also try changing dims
...
> ⌨🔥 Exercise: run the Algorithm with different number of PCs -> How does it infleunce the visualization?

```{r}
sample <- RunUMAP(sample, dims = 1:5, n.neighbors = 30, verbose = FALSE,
                  reduction.name = "umap_20PC" )
do_DimPlot(sample, reduction = "umap_20PC") # Also try changing dims
...
Hard to say, the less PCs -> Less Information but less noise. More PCs -> More Information and More noise

> Could you have the UMAP projection onto 3 axis instead of 2?
Yes. In Seurat you can also map to 3 or more Dimensions