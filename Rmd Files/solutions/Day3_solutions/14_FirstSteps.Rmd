...
For now we assume that this data resides in a directory  (relative to this current markdown file). You need to ensure that the path to your code
...
GEOquery provides functions to Download data directly from the Gene Expression omnibus and
allows direct access to metadata and data files tha have been uploaded there. 

Check out https://www.bioconductor.org/packages/release/bioc/vignettes/GEOquery/inst/doc/GEOquery.html to learn more. 
...
```{r}
#| eval: true
# https://github.com/gcapture/glioma-he
...
# Load samples.
mt_pattern <- "^MT-" # Regular Expression 
# NOTE: You need to change this to 
sample_path <- "/vol/data/course_data/data/filtered_feature_bc_matrix" 
# Path to where the count matrix from cellranger is located.
# Change according to where you have saved your data


# Read the count matrix and generate a Seurat object.
sample <- Seurat::Read10X(data.dir = sample_path)  # 
sample <- Seurat::CreateSeuratObject(sample, # Matrix file
                                     project = sample_name, # Project name 
                                     min.cells = 3, # Gene Filter (Expressed in 3 cells)
                                     min.features = 200 # Cell Filter
                                    )
...
```{r}
rowSums(sample@assays$RNA@layers$counts) -> rowsum_counts

print("Genes not expressed in any cell")
which(rowsum_counts == 0) %>% length()
...
```{r}
# We use rowsum Counts form above
# We add names to know which genes we have here

names(rowsum_counts) <- rownames(sample)

sort(rowsum_counts,decreasing = T)[1:3]
```
...
hist(sample@assays$RNA$counts[rownames(sample) == "MALAT1",],
     breaks = "FD",main = "MALAT1", xlab = "counts")
...
```{r}
colSums(sample@assays$RNA$counts) -> colsum_counts

which(colsum_counts < 2000) %>% length()
```
...
> Excercise: Now plot the distribution of the normalzed expression data for the Gene with the highest # of Counts. 

```{r}
hist(as.matrix(sample@assays$RNA$data["MALAT1",]), breaks = "FD",xlim = c(0.1,6))
```
...
```{r}
SCpubr::do_FeaturePlot(sample, reduction = "pca", features = "nCount_RNA")
```
