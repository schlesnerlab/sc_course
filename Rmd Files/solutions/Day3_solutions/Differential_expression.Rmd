...
celltype_de <- FindMarkers(sample, ident.1 = glue::glue("{cell_type}_2"),
                                    ident.2 = glue::glue("{cell_type}_3"),
                                    verbose = FALSE)
head(celltype_de, n = 10)
```
...
We run into Similar problems as before 
...
Volcanoplots between logFC and p-values allow assessment of significance and effect
size of results. Alternative is that we check the Featureplots of the Top genes


```{r}
sample$New_NMF_labelling %>% unique -> cell_types

cell_types <- head(cell_types, -1) # drop last entry
de_res <- list()
for (cell_t in cell_types) {
  cell_t
  celltype_de <- FindMarkers(sample, ident.1 = glue::glue("{cell_t}_3"),
                                    ident.2 = glue::glue("{cell_t}_2"),
                                    verbose = FALSE)
  de_res[[cell_t]] <- celltype_de
  celltype_de$label <- rownames(celltype_de)
  celltype_de$label <- ifelse(1:nrow(celltype_de) <= 10, celltype_de$label, "")
  ggplot(celltype_de, aes(x=avg_log2FC, y = -log10(p_val_adj), label = label)) + geom_point() +
    geom_label_repel()
}
```
...
In This case we see barely any expression differences, which we can expect to a certain degree
since the tumor grade shouldn't have a big influence on gene expression across samples. 


```{r}
SCpubr::do_ExpressionHeatmap(sample, features = common[1:20])

SCpubr::do_ExpressionHeatmap(sample, features = only_sc[1:20])

SCpubr::do_ExpressionHeatmap(sample, features = only_bulk[1:20])
```
